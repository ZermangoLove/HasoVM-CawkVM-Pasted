...
namespace HasoVM.Core.Protections.Mutations
{
    class Mutations : IProtection
    {
                        ...
                        for (int i = 0; i < method.Body.Instructions.Count; i++)
                        {
                            if (method.Body.Instructions[i].OpCode == OpCodes.Ldc_I4 && CanProtect(method.Body.Instructions, i))
                            {
                                ...
                                if (random.Next(0, 101) < 80)
                                    LocalMaker(module, method, i, ref fork);
                                i += fork;
                            }
                        }
                        method.Body.SimplifyBranches();
        ...
        private static void LocalMaker(ModuleDefMD module, MethodDef method, int i, ref int fork)
        {
            ...
            TypeDefUser structDef2 = new TypeDefUser(Utils.Rename(1), module.CorLibTypes.GetTypeRef("System", "ValueType"));
