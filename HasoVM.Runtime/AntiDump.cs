...
namespace Runtime
{
    class AntiDump
    {
	    ...
	    internal unsafe static extern void VirtualProtect(byte* lpAddress, int dwSize, uint flNewProtect, out uint lpflOldProtect);
	
		unsafe public static void InitAntiDump()
        {
			//From ConfuserEx 1.6.0
			
			uint old;
			
			var bas = (byte*)Marshal.GetHINSTANCE((typeof(AntiDump).Module));
			byte* ptr = bas + 0x3c;
			byte* ptr2;
			...
			if ((typeof(AntiDump).Module).FullyQualifiedName[0] != '<') //Mapped
			{
