...
namespace Runtime
{
	internal class JIT
	{
		...
		unsafe delegate void lambda__();

		private static lambda__ Run;
	
		static JIT()
        {
			byte[] array;
			using (Stream manifestResourceStream2 = typeof(JIT).Assembly.GetManifestResourceStream((IntPtr.Size == 4) ? "HasoRT" : "HasoRT64"))
			{
			...
			string text = Path.Combine(tempFileName, text1);
			File.WriteAllBytes(text, dec);
			IntPtr intPtr = b(text);
			IntPtr addr = c(intPtr, "RunHaso");
			Run = (lambda__)Marshal.GetDelegateForFunctionPointer(addr, typeof(lambda__));
		
		}
		public static unsafe void Hookx()
		{
			Run();
		}
